...
```mermaid
flowchart TD
    A[Start: Interested in<br/>Head Start] --> B{Child age 3-5?}

    B -->|No, 0-3| C[See Early Head Start<br/>instead]
    B -->|Yes| D{Family income<br/>at/below FPL?}

    D -->|Yes| E[Categorically<br/>eligible]
    D -->|No, but under 130%| F[May qualify in<br/>10% over-income slots]
    D -->|No, over 130%| G{Other qualifying<br/>factors?}

    G -->|Homeless| H[Automatically eligible]
    G -->|Foster care| H
    G -->|Receiving TANF/SSI| H
    G -->|Child has disability| I[10% disability slots<br/>No income requirement]
    G -->|None| J[Not eligible<br/>Consider CSPP]

    E --> K[Contact local<br/>Head Start grantee]
    F --> K
    H --> K
    I --> K

    K --> L[Complete application<br/>Provide documentation]

    L --> M{Slot available?}

    M -->|Yes| N[Enroll<br/>Begin comprehensive services]
    M -->|No| O[Join waitlist<br/>Priority based on need]

    O --> P{Slot opens?}

    P -->|Yes| N
    P -->|Consider| Q[Apply for CSPP, APP<br/>while waiting]

    click C "Early_Head_Start_Overview.md"
    click J "../CSPP/CSPP_Overview.md"
    click Q "../../04_Application_Processes/Application_Process_Overview.md"
```
